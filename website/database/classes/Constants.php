<?php

class Constants
{
    public static string $nameLengthErrorMessage = "Length must be between 2 and 25 characters";
    public static string $emailInvalidErrorMessage = "Invalid email";
    public static string $emailTakenErrorMessage = "Email already in use";
    public static string $phoneNumberInvalidErrorMessage = "Invalid phone number";
    public static string $phoneNumberTakenErrorMessage = "Phone number already in use";
    public static string $passwordsMissMatchErrorMessage = "Passwords dont match";
}